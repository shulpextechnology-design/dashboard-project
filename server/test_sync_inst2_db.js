...
async function syncInstance2() {
  console.log('Testing Instance 2 sync...');
  ...
  const contentRes = await client.get('https://members.freelancerservice.site/content/p/id/45/', { timeout: 20000 });
  const tokenMatch = contentRes.data.match(/(?:var\s+)?copyText\s*=\s*["']\s*(brandseotools.*?)\s*["']/s);
  const token = tokenMatch ? tokenMatch[1] : null;

  console.log('Instance 2 token extracted! Length:', token ? token.length : 0);
  if (token) {
    ...
    console.log('Instance 2 DB updated to Success!');
  }
}

syncInstance2().catch(console.error);

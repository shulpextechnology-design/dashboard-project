...
async function fixInstance2() {
    ...
    console.log('[Fix] Logging in to freelancerservice.site...');
    ...
    console.log('[Fix] Fetching Instance 1 token...');
    const res1 = await client.get('https://members.freelancerservice.site/content/p/id/173/', { timeout: 20000 });
    const token1 = (res1.data.match(/(?:var\s+)?copyText\s*=\s*["']\s*(brandseotools.*?)\s*["']/s) || [])[1];

    console.log('[Fix] Fetching Instance 2 token...');
    const res2 = await client.get('https://members.freelancerservice.site/content/p/id/45/', { timeout: 20000 });
    const token2 = (res2.data.match(/(?:var\s+)?copyText\s*=\s*["']\s*(brandseotools.*?)\s*["']/s) || [])[1];
    ...
    if (token1) {
        ...
        console.log('[Fix] ✅ Instance 1 updated in DB!');
    }
    ...
    if (token2) {
        ...
        console.log('[Fix] ✅ Instance 2 updated in DB!');
    }
}

fixInstance2().catch(console.error);
